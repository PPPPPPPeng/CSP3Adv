...
class ClassificationImageData:
    ...
    def add_record(self, img, label, writer):
        img = to_rgb(img)
        img = np.array(Image.fromarray(img).resize([self.img_size, self.img_size])).astype(np.uint8)
    ...
    def write_tfrecord_from_mxrec(self, read_dir, write_path):
        # generate TF Record function
        import mxnet as mx
        print('Start write tfrecord from mxrec...')
