...
def get_args():
    ...
    parser.add_argument('--config_path', type=str, default='/data/jiaming/code/APF/configs/config_ms1m_100.yaml', help='config path, used when mode is build')
...
def get_embd(inputs, config, reuse=tf.AUTO_REUSE, scope='embd_extractor'):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if config['backbone_type'].startswith('resnet_v2_m'): # m
            ...
            with slim.arg_scope(arg_sc): # 进行一些默认值设定
        ...
        if config['out_type'] == 'E': # ensemble?
            with slim.arg_scope(arg_sc):
                ...
                # net = tf.keras.layers.GlobalAveragePooling2D()(net) # raise ValueError: The last dimension of the inputs to `Dense` should be defined. Found `None`.
                net = slim.fully_connected(net, config['embd_size'], normalizer_fn=None, activation_fn=None) # embd_size: 512 (arcface)
...
# def input_diversity(input_tensor): # rescale and padding
